...
export type GameCategory = 'quiz' | 'trivia' | 'word' | 'puzzle' | 'memory' | 'multiplayer' | 'arcade' | 'strategy';
...
export type GameSessionStatus = 'created' | 'active' | 'paused' | 'completed' | 'expired' | 'terminated';
/**
 ...
 */
export type GameEventType = 'game_started' | 'game_paused' | 'game_resumed' | 'game_progress' | 'game_score_updated' | 'game_completed' | 'game_error' | 'custom';
 ...
 */
export interface GameModule<TConfig = Record<string, unknown>, TAction = GameAction, TState = GameState, TResult = GameResult> {
    id: string;
 ...
 * Webhooks
 */
export type WebhookEventType = 'game.session.created' | 'game.session.started' | 'game.session.completed' | 'game.result.created' | 'game.session.expired';
